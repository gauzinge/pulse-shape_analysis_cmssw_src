...
namespace sistrip {
  ...
  {
    std::cout<<"Source code in /opt/cmssw/Stable/current/src/DQM/SiStripMonitorHardware called"<<std::endl;
  ...
  bool FEDSpyBuffer::delayChipGood(const uint8_t delayChip) const
  {
    ...
    if (delayChip == 0) {
      ...
      s1<<"delayChip == 0 "<<" totalEventCountBefore from spyHeaderTotalEventCount() is "<<totalEventCountBefore<<std::endl;
      dump(s1);
    } else {
      std::ostringstream s2;
      l1CountBefore = delayChipL1ID(delayChip-1);
      totalEventCountBefore = delayChipTotalEventCount(delayChip-1);
      s2<<"delayChip != 0 "<<" totalEventCountBefore from delayChipTotalEventCount(delayChip-1) is "<<totalEventCountBefore<<std::endl;
      dump(s2);
    }

  ...
  run_(run)
{
  ...
  std::cerr << "CalibrationScanTask::CalibrationScanTask for " << conn.detId() << " pedrange = " << detPedRange.second-detPedRange.first << " meaning " << (detPedRange.second-detPedRange.first)*8/10 << " Strips, start = "<<start<<" stop = "<<stop<<std::endl;
  int maxstrip = (detPedRange.second-detPedRange.first)*8/10;
...
//  LogDebug("Commissioning") << "[CalibrationScanTask::fill]: isha/vfs = " << summary.isha() << "/" << summary.vfs();
  ...
  std::cerr << "CalibrationScanTask::fill latency="<<summary.latency()<<" calSwl="<<summary.calSel()<<" bin="<<bin<<std::endl;

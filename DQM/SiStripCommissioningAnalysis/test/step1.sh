...
# for i in `ls /afs/cern.ch/user/e/ebutz/work/CalibScan/${1} | grep SiStripCommissioningSource`; { echo -n "-M /castor/cern.ch/user/d/delaer/CMStracker/${1}/$i "; } | xargs -n1000 -s129000 stager_get -U CDrun${1}
...
bsub -q 1nh "`pwd`/step1bsub.sh ${1} ${2} ${isha} ${vfs} `pwd`"
done
done

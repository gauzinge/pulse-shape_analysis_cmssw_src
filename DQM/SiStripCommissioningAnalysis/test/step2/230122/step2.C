void step2(int runnumber, int ishalow, int ishahigh, int ishastep, int vfslow, int vfshigh, int vfsstep)
{
  ...
  CalibrationScanAnalysis analysis;
  {
	...
	//        analysis.addFile(Form("/afs/cern.ch/work/e/ebutz/CalibScan/229970/SiStripCommissioningClient_00%d_ISHA%d_VFS%d.root",runnumber,isha,vfs));
        analysis.addFile(Form("/afs/cern.ch/work/d/dstrom/CalibScan/230122/SiStripCommissioningClient_00%d_ISHA%d_VFS%d.root",runnumber,isha,vfs));
  }
  analysis.analyze();
